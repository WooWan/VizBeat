...
export default function Bar({ radius, index, centerPos, position, theta, color, meanRef, dataArrayRef }: Props) {
  ...
  useFrame((_) => {
    ...
    const nx = centerPos[0] + (radius + Math.round(power * 5) * 10) * Math.cos(angleRef.current);
    const ny = centerPos[1] + Math.log(1 + heightRef.current * 200) * 10;
    const nz = centerPos[2] + (radius + Math.round(power * 5) * 10) * Math.sin(angleRef.current);
