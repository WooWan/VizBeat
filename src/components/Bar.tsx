...
export default function Bar({ radius, index, centerPos, position, theta, color, meanRef, dataArrayRef }: Props) {
  ...
  useFrame((_) => {
    ...
    const ny = centerPos[1] + sigmoid(heightRef.current * 20) * 70;
    // console.log(heightRef.current * heightRef.current * 3000);
    // const ny = centerPos[1] + sigmoid(heightRef.current * heightRef.current) * 3000;
